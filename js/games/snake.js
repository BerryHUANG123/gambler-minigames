...
(function() {
  ...
  BaseGame.init('snake', '🐍', '贪吃蛇', {
    tableHTML: `<div id="snGrid" style="display:grid;grid-template-columns:repeat(${SIZE},1fr);gap:1px;max-width:300px;margin:8px auto;background:#111;padding:2px;border-radius:4px;"></div><div style="font-size:0.9rem;">得分：<span id="snScore" style="color:var(--gold);">0</span></div><div id="snResult" class="message">方向键或按钮控制</div>`,
    controlsHTML: `<div style="display:grid;grid-template-columns:repeat(3,40px);gap:4px;margin:6px auto;"><button class="btn btn-sm" style="grid-column:2;" onclick="SN.setDir(0,-1)">↑</button><button class="btn btn-sm" onclick="SN.setDir(-1,0)">←</button><button class="btn btn-sm" onclick="SN.setDir(0,1)">↓</button><button class="btn btn-sm" onclick="SN.setDir(1,0)">→</button></div><button class="btn btn-primary" id="snStartBtn" onclick="SN.start()">开始！</button>`
  });
  window.SN={
    bet: BaseGame.betHandler('snake', state),
