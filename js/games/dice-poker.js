(function() {
  ...
  BaseGame.init('dice-poker', '🎲', '骰子梭哈', {
    tableHTML: '<div class="hand" id="dpDice" style="gap:10px;"></div><div id="dpRank" style="font-size:1.1rem;color:var(--gold);min-height:30px;"></div><div id="dpResult" class="message">掷出好组合赢钱！</div>',
    controlsHTML: '<button class="btn btn-primary" id="dpRollBtn" onclick="DP.roll()">🎲 掷骰子</button>'
  });
  window.dice_pokerBet = function(amount) {
    if (state.gameOver) return;
    if (!Engine.canBet(amount)) return;
    BaseGame.betHandler('dice-poker', state)(amount);
    if (state.bet > 0 && state.rolls === 0) DP.init();
  };
  window.DP = {
    ...
    render() {
      ...
      if (rank.mult >= 3) { res.textContent = `🎉 ${rank.name}！赢 ${win} 筹码！`; res.className = 'message msg-win'; Engine.showQuote('win'); }
      else { res.textContent = `${rank.name}，得 ${win} 筹码`; res.className = rank.mult > 1 ? 'message msg-win' : 'message'; }
      BaseGame.settle('dice-poker', state, true, win);
