...
(function() {
  ...
  BaseGame.init('fifteen', '🧩', '数字华容道', {
    tableHTML: '<div id="ftGrid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:3px;max-width:220px;margin:8px auto;"></div><div style="font-size:0.9rem;">步数：<span id="ftMoves" style="color:var(--gold);">0</span></div><div id="ftResult" class="message"></div>',
    controlsHTML: '<button class="btn btn-primary" id="ftStartBtn" onclick="FT.start()">开始！</button>'
  });
  window.FT={
    bet: BaseGame.betHandler('fifteen', state),
    ...
    move(i){
      ...
      if(isSolved(state.grid)){state.gameOver=true;state.started=false;const bonus=Math.max(1,50-state.moves);const win=state.bet*bonus;BaseGame.settle('fifteen',state,true,win);document.getElementById('ftResult').textContent=`🎉 ${state.moves}步拼好！赢 ${win}！`;document.getElementById('ftResult').className='message msg-win';Engine.showQuote('win');document.getElementById('ftStartBtn').disabled=false;}
    }
  };
})();
